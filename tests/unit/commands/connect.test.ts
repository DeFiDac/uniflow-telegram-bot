...
import {
	createMockMessage,
	mockMessageWithoutFrom,
	mockMessageWithoutChat,
} from '@tests/fixtures/telegram-messages';
import {
	mockPrivyUser,
	mockPrivyUserWithWallet,
	mockWallet,
} from '@tests/fixtures/privy-responses';
	...
	describe('Happy path - New user', () => {
		it('should create new Privy user when none exists', async () => {
			...
			expect(mockPrivy.users().create).toHaveBeenCalledWith({
				linked_accounts: [{ type: 'telegram', telegram_user_id: '789' }],
			});
